...
FixedRateCoupon(paymentDate::Date, faceAmount::Float64, rate::IR, 
            accrual_start::Date, accrual_end::Date,
            ref_start::Date, ref_end::Date, 
            dc::DC, accrual::Float64) where {
                DC <: DayCount, IR <: InterestRate
            } = 
    #BoB equal to 
    FixedRateCoupon{DC, IR}(CouponMixin{DC}(accrual_start, accrual_end,
                                            ref_start, ref_end, dc, accrual),
                            paymentDate, faceAmount, rate)

function amount(coup::FixedRateCoupon)
    return coup.nominal * (
                            compound_factor(coup.rate, 
                                accrual_start(coup), accrual_end_date(coup), 
                                ref_period_start(coup), ref_period_end(coup)) 
                            -1.0
                            )
end
...
function FixedRateLeg(schedule::Schedule, faceAmount::Float64,rate::Float64, 
                calendar::BusinessCalendar, paymentConvention::BusinessDayConvention,
...
function FixedRateLeg(schedule::Schedule, faceAmount::Float64, rates::Vector{Float64}, 
                calendar::BusinessCalendar, paymentConvention::BusinessDayConvention, dc::DC;
    ...
    coups[1] = FixedRateCoupon(payment_date, faceAmount, 
                            InterestRate(rates[1], dc, SimpleCompounding(), schedule.tenor.freq), 
                            start_date, end_date, start_date, end_date, dc, -1.0)
    count = 2
    start_date = end_date
    end_date = count == length(schedule.dates) ? schedule.dates[end] : schedule.dates[count + 1]
    payment_date = adjust(calendar, paymentConvention, end_date)
    while start_date < schedule.dates[end]
        @inbounds coups[count] = FixedRateCoupon(payment_date, faceAmount, 
                InterestRate(rates[count], dc, SimpleCompounding(), schedule.tenor.freq), 
                start_date, end_date, start_date, end_date, dc, -1.0)
